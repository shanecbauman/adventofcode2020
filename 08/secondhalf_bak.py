...
class run_instructions:
    ...
    def change_instruction(self, instruction, index):
        if self.found_new_path == True:
            return instruction
        elif self.instructions[index][4] == True:
            if instruction[0] == 'acc' or instruction[0] == 'jmp':
                return instruction
            else:
                return [
                    'nop',
    ...
    def execute_instructions(self, index):
        if self.instructions[index][3] == True:
            ...
            if self.instructions[index][0] == 'acc':
                ...
                self.execute_instructions(index + 1)
            elif self.instructions[index][0] == 'nop':
                if not self.execute_instructions(index):
                    self.execute_instructions(index + self.instructions[index[2]])
            elif self.instructions[index][0] == "jmp":
                if not self.execute_instructions(index):
                    self.execute_instructions(index + 1)
                # self.execute_instructions(index + 1)
            index = self.og_index
            # increment_instructions = self.change_instruction(self.instructions[index], index)
            # index = self.indexer(index, int(increment_instructions[2]), increment_instructions[1])
